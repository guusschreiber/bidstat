...
check_dataset(Dataset) :-
  ...
  maplist(check_record, IDs, Errors),
  sum_list(Errors, NumErrors), 
  format(user, "Number of erroneous deal records found: ~a~n", 
    [NumErrors]), 
  check_equal_deals,
  close(ErrorStream). 

check_record(ID, HasError) :-
...
check_equal_deals :-
  forall(same_deal(ID1, ID2), check_equal_deal(ID1, ID2)).
