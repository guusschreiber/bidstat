...
process_dataset(Dataset, Collections) :-
  ...
  find_equal_deals, 
  format(user, "Total number of deals processed: ~a~n", [Overall]),  
  close(DataStream).

find_equal_deals :-
  findall(ID, deal(ID, _), IDs), 
  maplist(find_equal_deal, IDs).

find_equal_deal(ID1) :-
  \+ deal_feature(ID1, hands(_)).
find_equal_deal(ID1) :-
  deal_feature(ID1, hands(H)),
  ( hands(ID2, H)
  , write_term(data, equal_deal(ID2, ID1), [fullstop(true), nl(true)])
  , assert(equal_deal(ID2, ID1))
  ; asserta(hands(ID1, H))
  ). 

deal_feature(ID, Feature) :-
  deal(ID, Facts),
...
process([Path, Collection], Total) :-
  ...
  read(input, Deal), 
  process_deal(Collection, Deal, 0, Total),
  format(user, "Collection ~w processed: ~a deals~n", 
         [Collection, Total]), 
  close(ReadStream).

process_deal(_, end_of_file, Total, Total).
process_deal(Collection, deal([end_of_deal]), ID, Total) :-
  read(input, Deal), 
  process_deal(Collection, Deal, ID, Total).
process_deal(Collection, deal(Args), PrevID, Total) :-
  ID is PrevID + 1,   
  process_args(Args, Facts),
  atomic_list_concat([Collection, '_', ID], DealID),
  ( \+ ground(deal(DealID, Facts))
  , format(user, "Not ground: ~w ~w ~n", [DealID, Facts])
  ; assert(deal(DealID, Facts))
  , portray_clause(data, deal(DealID, Facts))
  , deal_def(DealID)
  ), 
  read(input, Deal),
  process_deal(Collection, Deal, ID, Total).

process_args(Args, DealProps) :-
  ...
  flatten([FeatureValues, DerivedValues], DealProps).
