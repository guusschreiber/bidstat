...
stat(Dataset, StatFeature) :-
  ...
  close(Stream). 
stat(Dataset, StatFeature) :-
  set_profile(StatFeature, Profile), 
  memberchk(labels(Labels), Profile), 
  init_stat(Dataset, StatFeature, Labels, Stream),   
  assert(required_info(result)),
  stat_all_sets(StatFeature, Profile),
  format(user,"Data table generated: ~a~n",[StatFeature]),
  close(Stream). 

stat_all_deals(StatFeature) :-
  forall(deal(ID, _), 
    ( ( \+ suitable_deal(ID, StatFeature)
      ; deal_profile(StatFeature, ID, Values)
      , output_row(ID, Values)
      ))).

stat_all_sets(StatFeature, Profile) :-
  findall(ID, equal_deal(ID, _), IDs), 
  list_to_set(IDs, SetIDs), 
  maplist(stat_set(StatFeature, Profile), SetIDs). 

stat_set(StatFeature, Profile, SetID) :-
  once(deal_set(StatFeature, SetID, Set)),
...
find_mean_imp_diff(Set, Hand, BidList, [N1, N2], Imps) :-
  ( [Bid1, Bid2] = BidList 
  , find_deals(Set, Hand, Bid1, SubSet1) 
  , find_deals(Set, Hand, Bid2, SubSet2)
  ; [Bid] = BidList
  , find_deals(Set, Hand, Bid, SubSet1) 
  , find_deals_complement(Set, Hand, Bid, SubSet2) 
  ), 
  SubSet1 \= [], SubSet2 \= [], 
  findall(Imps, 
    ( member(ID1, SubSet1)
...
find_deals(Set, Hand, Bid, SubSet) :-
  findall(ID, 
    ( member(ID, Set)
    , bid(ID, Hand, 1, Bid, _))
    , SubSet).
find_deals_complement(Set, Hand, Bid, SubSet) :-
  findall(ID, 
    ( member(ID, Set)
    , \+ bid(ID, Hand, 1, Bid, _))
    , SubSet).

deal_set(StatFaeture, SetID, Set) :-
  findall(ID, equal_deal(SetID, ID), IDs), 
  flatten([SetID, IDs], FullSet), 
  findall(ID, 
    ( member(ID,FullSet),suitable_deal(ID,StatFaeture)), 
    Set),
  length(Set, L), 
  L > 1. 
...
deal_profile(opening_1st_hand, ID, [Bool]) :-
  bid(ID, 1, 1, _, Features), !,
  ( memberchk(opening, Features)
  , Bool is 1
  ; Bool is 0
  ).  
deal_profile(opening_1st_hand, _, ['NA']).
...
deal_profile(contested_auction, ID, [Bool, NBids, Length]) :-
...
deal_profile(bal_1st_hand, ID, [1]) :-
  hand(ID, 1, _, _, Features),
  memberchk(balanced, Features). 
deal_profile(bal_1st_hand, _, [0]).
...
deal_profile(opening_11_12_bal_1, ID, [Bool]) :-
...
deal_profile(opening_11_12_bal_2, ID, [Bool]) :-
  ...
  , Bool is 1
  ; Bool is 0 ). 
deal_profile(opening_11_12_bal_2, _, ['NA']).
...
deal_profile(weak_two_1st, ID, [Value]) :-
  hand(ID, 1, HCP, [_, [6|_]], _), 
  HCP < 10, 
  bid(ID, 1, 1, Bid, _), 
  Value is Bid div 10. 
deal_profile(weak_two_1st, _, ["NA"]).

deal_profile(weak_two_1st_nv, ID, [Value]) :-
  deal_feature(ID, vul(Vul)),
  ( Vul == 1 ; Vul == 3), 
  hand(ID, 1, HCP, [_, [6|_]], _), 
  HCP < 10, 
  bid(ID, 1, 1, Bid, _), 
  Value is Bid div 10. 
deal_profile(weak_two_1st_nv, _, ["NA"]).
...
deal_profile(game_hcp, ID, [1, GameType, IsVul, HCP, Made]) :-
  deal_feature(ID, contract([Level, Suit])), 
  is_game([Level, Suit], GameType), 
  deal_feature(ID, declarer(Declarer)), 
  deal_feature(ID, vul(DealVul)), 
  is_vul(Declarer, DealVul, IsVul), 
  the_partner(Declarer, Partner), 
  hand(ID, Declarer, HCP1, _, _), 
  hand(ID, Partner, HCP2, _, _),
  HCP is HCP1 + HCP2,
  deal_feature(ID, result(Result)), 
  ( Result < Level + 6
  , Made is 0
  ; Made is 1). 
deal_profile(game_hcp, _, [0, 'NA','NA','NA','NA']).
